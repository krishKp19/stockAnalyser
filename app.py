...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("<p class='version-text'>v6.5 | Triple-Redundant Hybrid Engine</p>", unsafe_allow_html=True)
...
# --- LAZY CHARTING (120 DAYS) ---
def plot_technical_chart(hist, ticker):
    import plotly.graph_objects as go
    from plotly.subplots import make_subplots

    fig = make_subplots(rows=2, cols=1, shared_xaxes=True, 
                        vertical_spacing=0.03, subplot_titles=(f'{ticker} Price (Last 120 Days)', 'Volume'), 
                        row_heights=[0.7, 0.3])
...
# --- DATA ENGINE (HYBRID WITH WATERFALL FALLBACKS) ---
@st.cache_data(ttl=3600)
def get_market_data(ticker):
    try:
        clean_ticker = ticker.upper().replace('.NS', '').replace('.BO', '')
        ...
        # ==========================================
        # PART 1: FETCH FUNDAMENTALS (SCREENER.IN)
        # ==========================================
        url = f"https://www.screener.in/company/{clean_ticker}/consolidated/"
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'}
        ...
        # ==========================================
        # PART 2: THE TRIPLE-REDUNDANT CHART ENGINE
        # ==========================================
        hist = None
        rsi_val, trend, vol_ratio = 50, "N/A", 1.0
        chart_source = "None"
        
        # We need 250 days to calculate 200 DMA, then we slice it to 120 days for display.
        fetch_bars = 250 
        display_bars = 120
        ...
        try:
            ...
            hist_tv = tv.get_hist(symbol=clean_ticker, exchange='NSE', interval=Interval.in_daily, n_bars=fetch_bars)
            if hist_tv is not None and not hist_tv.empty:
                hist = hist_tv.rename(columns={'open': 'Open', 'high': 'High', 'low': 'Low', 'close': 'Close', 'volume': 'Volume'})
                chart_source = "TradingView"
        except Exception: pass

        # Attempt 2: YFinance (If TradingView fails)
        if hist is None:
            try:
                import yfinance as yf
                yf_ticker = f"{clean_ticker}.NS"
                hist_yf = yf.Ticker(yf_ticker).history(period="1y") # 1y is approx 252 trading days
                if not hist_yf.empty:
                    hist = hist_yf
                    chart_source = "Yahoo Finance"
            except Exception: pass

        # Attempt 3: YahooQuery (If YFinance fails)
        if hist is None:
            try:
                from yahooquery import Ticker as YQTicker
                yq_ticker = f"{clean_ticker}.NS"
                hist_yq = YQTicker(yq_ticker).history(period="1y")
                if not isinstance(hist_yq, dict) and not hist_yq.empty:
                    if isinstance(hist_yq.index, pd.MultiIndex):
                        hist_yq = hist_yq.xs(yq_ticker)
                    hist = hist_yq.rename(columns={'open': 'Open', 'high': 'High', 'low': 'Low', 'close': 'Close', 'volume': 'Volume'})
                    chart_source = "YahooQuery"
            except Exception: pass

        # --- PROCESS TECHNICALS & SLICE TO 120 DAYS ---
        if hist is not None and not hist.empty:
            # 1. Calculate indicators on the FULL 250-day dataset
            hist['SMA_50'] = hist['Close'].rolling(window=50).mean()
            hist['SMA_200'] = hist['Close'].rolling(window=200).mean()
            ...
            # 2. SLICE the dataset to only the last 120 days for the UI
            hist = hist.tail(display_bars)

        # Final Signal Calc
        signals = calculate_signals(vol_ratio, 0.15, 0.20, promoter)
...
def analyze_stock(gemini_key_param, model_name, data):
    ...
    prompt = f"""
    Act as a Senior Hedge Fund Analyst. Audit {data['Symbol']} using this 7-PHASE FRAMEWORK.
    DATA SOURCE: SCREENER.IN & {data['Chart_Source']}
    SECTOR CONTEXT: {data['Sector_Info']['Advice']}
...
# --- MAIN UI ---
st.title("📈 AI Hedge Fund Terminal")
st.caption("Institutional Grade Forensic Analysis • Screener + Hybrid Chart Engine")
...
if submitted:
    if not gemini_api_key:
        ...
        with st.spinner(f"Scraping Fundamentals & Technicals for {ticker}..."):
            data, hist = get_market_data(ticker)
            
            if data is not None:
                st.success(f"✅ DATA EXTRACTED SUCCESSFULLY (Chart Engine: {data['Chart_Source']})")
                ...
                st.divider()
                if hist is not None:
                    st.subheader("📉 Technical Breakout Check (120 Days)")
                    st.plotly_chart(plot_technical_chart(hist, ticker), use_container_width=True)
                else:
                    st.info("⚠️ Live chart currently unavailable due to rate limits. Showing fundamental analysis below.")
